      ...
      <div className="mx-auto max-w-7xl">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Link href={`/vr/${project.id}?preview=1`} target="_blank" className="rounded border border-stone-300 bg-white px-4 py-2 text-sm">
              后台预览
            </Link>
            {project.status === "PUBLISHED" && (
              <Link href={`/vr/${project.id}`} target="_blank" className="rounded bg-ink px-4 py-2 text-sm text-white">
                打开发布页
              </Link>
            )}
          </div>
        ...
        <div className="mb-6 grid gap-3 md:grid-cols-5">
          {["上传图纸", "自动识别", "风格预览", "批量生成", "复核发布"].map((step, index) => (
            <div key={step} className="rounded border border-stone-200 bg-white p-3 text-sm shadow-sm">
              <span className="mr-2 inline-grid h-6 w-6 place-items-center rounded-full bg-fern text-xs text-white">{index + 1}</span>
              {step}
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[410px_1fr]">
          <aside className="space-y-6">
            ...
            <section className="rounded border border-stone-200 bg-white p-4 shadow-sm">
              <h2 className="font-semibold">上传格式要求</h2>
              <div className="mt-4 space-y-4">
                {uploadRequirements.map((item) => (
                  <details key={item.type} open={item.type === "IMAGE"} className="rounded bg-stone-50 p-3">
          ...
          <section className="space-y-6">
            <div className="rounded border border-stone-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="font-semibold">生成控制</h2>
              ...
              <InfoPanel title="户型识别结果">
                ...
                <div className="mt-3 space-y-2">
                  {project.rooms.map((room) => (
                    <div key={room.id} className="flex justify-between rounded bg-stone-50 px-3 py-2 text-sm">
                      <span>{room.order}. {room.name}</span>
                      <span className="text-stone-500">{room.roomType} / {Math.round(room.confidence * 100)}%</span>
                    </div>
                  ))}
                  {project.rooms.length === 0 && <p className="text-sm text-stone-500">尚未识别到房间。</p>}
                </div>
              </InfoPanel>
              ...
              <InfoPanel title="AI 对话记录">
                <div className="space-y-2">
                  {project.conversations.map((message) => (
                    <div key={message.id} className="rounded bg-stone-50 px-3 py-2 text-sm">
                      <span className="font-medium">{message.role === "USER" ? "用户" : "AI"}：</span>
                      {message.content}
                    </div>
                  ))}
                  {project.conversations.length === 0 && <p className="text-sm text-stone-500">还没有风格调整记录。</p>}
                </div>
              </InfoPanel>
            </div>
            ...
            <InfoPanel title="房间生成结果">
              <div className="grid gap-4 md:grid-cols-2">
                {project.scenes.map((scene) => (
                  <article key={scene.id} className="overflow-hidden rounded border border-stone-200">
                    <img src={scene.thumbnailUrl} alt={scene.name} className="h-44 w-full object-cover" />
                    <div className="p-3">
                      <h3 className="font-medium">{scene.name}</h3>
                      <p className="text-xs text-stone-500">{scene.renderMode}</p>
                    </div>
                  </article>
                ))}
              </div>
              {project.scenes.length === 0 && <p className="text-sm text-stone-500">尚未生成房间效果图。</p>}
            </InfoPanel>

            <InfoPanel title="最近生成任务">
              <div className="space-y-2">
                {project.jobs.slice(0, 4).map((job) => (
                  <div key={job.id} className="rounded bg-stone-50 p-3 text-sm">
                ...
                {project.jobs.length === 0 && <p className="text-sm text-stone-500">尚无生成任务。</p>}
              </div>
            </InfoPanel>
          </section>
        </div>
